...
class songAPI:
    ...
    async def stop(self, ctx):
        voice_client = get(self.bot.voice_clients, guild=ctx.guild)
        if voice_client == None:
            await ctx.channel.send("Bot is not connected to {0}".format(voice_client.channel))
            return
    ...
    async def pause(self, ctx):
        voice_client = get(self.bot.voice_clients, guild=ctx.guild)
        if voice_client == None:
            await ctx.channel.send("Bot is not connected to channel")
            return

        if voice_client.channel != ctx.author.voice.channel:
            await ctx.channel.send("The bot is currently connected to channel")
            return
    ...
    async def queueList(self, ctx):
        ...
        if voice_client == None or not voice_client.is_connected():
            await ctx.channel.send("Bot is not connected to channel", delete_after=10)
            return
        ...
        embed = discord.Embed(title=f'Upcoimg - Next {len(upcoming)}', description=fmt)
